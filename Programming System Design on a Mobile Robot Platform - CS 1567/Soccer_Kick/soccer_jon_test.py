...
def detect_blob():
    ...
    while(True):
        if firstTurn:
            os.system('spd-say \"Starting to turn\"')
            #rospy.sleep(6)
            currentAngle = 0
            currentDistance = 0
            resetOdom.publish("")
        ...
        if moveToKick:
                ...
                os.system('spd-say \"Lets kick this ball\"')
                ...
                resetOdom.publish("")
                moveToKick = False
                os.system('spd-say \"Ten points to griff in door\"')
